 ...
 * so there is exactly one answer. Without it, several services queried the
 * bundled simulator while the map read the socket — which meant the journey
 * planner could quote a departure time the live map disagreed with, on the same
 * screen, for the same bus.
 *
 * The board filter itself is `departuresAtStop` from the simulator module: it is
 * a pure function over a fleet array, so it serves the live feed and the offline
 * fallback identically rather than being reimplemented once per source.
 */
...
/** Services approaching a stop, soonest first, from whichever source is live. */
export function departuresAt(stopId: string, limit = 8): LiveDeparture[] {
  return departuresAtStop(stopId, liveSnapshot()).slice(0, limit);
}
